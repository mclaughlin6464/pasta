...
def make_kils_command(jobname,N,B0, xb, xp, n_steps, max_time = max_time, outputdir = outputdir, queue='kipac-ibq'):#'bulletmpi'):
    ...
    command = ['bsub',
               ...
               '-W', '%d:00' % max_time,
               'python', path.join('/u/ki/swmclau2/Git/pasta/pasta/', 'charged_ising.py'),
                str(N), str(B0), str(xb), str(xp), str(n_steps), outputdir]
...
for xb in xbs:
    for xp in xps:
        ...
        command = make_kils_command(jobname, N,B0, xb, xp, n_steps)
